...
class NotificationManager:
    ...
    def send_emails(self, email, message_body):
        with SMTP("smtp.gmail.com", 587) as connection:
            connection.starttls()
            connection.login(self.from_email, self.password)
            connection.sendmail(from_addr=self.from_email, to_addrs=email, msg=f"Subject:Don't miss your Dream Trip\n\n{message_body}")
            print("the connection was OK")