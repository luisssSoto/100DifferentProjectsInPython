...
def register():
    ...
    if form.validate_on_submit():
        ...
        if user:
            flash('You\'ve already created an account, log in instead')
            return redirect(url_for('login'))
        plain_password = form.user_password.data
        hash_salted_pwd = generate_password_hash(password=plain_password, method='pbkdf2:sha256', salt_length=8)
        user = User(
...
def login():
    ...
    if form.validate_on_submit():
        user_email = form.user_email.data
        user_plain_password = form.user_password.data
        print(user_plain_password)
        user = db.session.execute(db.select(User).where(User.email == user_email)).scalar()
        print(user)
        if user:
            if check_password_hash(pwhash=user.password, password=user_plain_password):
            ...
            return redirect(url_for('login'))
    return render_template("login.html", login_form=form, logged_in=current_user.is_authenticated)
